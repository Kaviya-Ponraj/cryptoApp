...
const SavedBtn = ({data}) => {
  ...
  const {savedCoin, allCoins, removeCoin} = useContext(StorageContext)
...
// console.log(cryptoData);
  return (
    <>
    <div className='flex flex-col mt-9 border rounded'>
      {
        cryptoData ? 
        
        <table className='w-full table-auto min-h-[70%]'>
            ...
            <tbody>
              {
                cryptoData.map((data) => {
                  return <tr key={data.id} className='text-center border-b border-gray-100 hover:bg-gray-300 bg-gray-200 last:border-b-0'>
                  <td className='py-4 flex items-center capitalize'>
                    <SavedBtn data={data}/>
