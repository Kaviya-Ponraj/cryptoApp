...
const HighLowIndicator = (current_price, high, low) => {
    ...
    return(<>
    <span className="bg-red rounded-l-lg w-[50%] h-1.5" style={{width : `${100 - green}%`}}>&nbsp;</span>
    <span className="bg-green rounded-r-lg w-[50%] h-1.5" style={{width : `${green}%`}}>&nbsp;</span>
    </>)
}
...
const CryptoDetails = () => {
    ...
    // console.log("from cryptodetails component", cryptoData)
            ...
            data ? (
                ...
                <div className="flex items-center justify-center h-full w-full p-4">
                    <div className="flex flex-col w-[45%] h-full pr-2 ">
                        ...
                        <div className={`text-sm px-1 ml-2 font-medium flex items-center rounde uppercase bg-opacity-25 ${data.market_data.price_change_percentage_24h > 0 ? `bg-green text-green` : `bg-red text-red`}
                        `}> <span className="">{Number(data.market_data.price_change_percentage_24h).toFixed(2)}%</span> 
                        <svg 
                        width="14" 
                        height="14" 
                        viewBox="0 0 14 14" 
                  ...
                  new Intl.NumberFormat("En-In", {
                    ...
                    <div className=" flex flex-col w-[55%] h-full pl-3 ">
                        ...
                        <div className="flex flex-col mt-4">
                            <h3 className=""> <span className="text-sm text-gray-100">Market Cap Rank : </span> {data.market_cap_rank === null ? "data not found" : data.market_cap_rank} </h3>
                        </div>

                        <div className="flex ml-2">
                            <span className="text-sm text-gray-100 ml-0">
                            Link to HomePage :  &nbsp;
                            </span>

                            <a className='cursor-pointer underline' href={data.links.homepage} target="_blank" rel="noopener noreferrer">
                                    { data.name}
                                </a> 
                        </div>
                    </div>
